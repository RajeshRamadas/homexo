...
function appendMessage(role, content) {
    ...
    // Sanitise AI-generated links: strip absolute dev-server URLs & remove /developers/ links
    const cleanedContent = sanitizeAiLinks(content);
...
function renderInlinePropertyCards(messageDiv, cards) {
    ...
    cards.forEach(p => {
        ...
        const imgHtml = rawImg
            ? `<img src="${rawImg}" alt="${escapeHtml(p.title)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" style="width:100%;height:120px;object-fit:cover;">
               <div class="urvashi-card-img-placeholder" style="display:none;width:100%;height:120px;background:#f0f4fa;align-items:center;justify-content:center;">
                 <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#0D2B4E" stroke-width="1.5" opacity="0.4"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
               </div>`
            : `<div class="urvashi-card-img-placeholder" style="display:flex;width:100%;height:120px;background:#f0f4fa;align-items:center;justify-content:center;">
                 <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#0D2B4E" stroke-width="1.5" opacity="0.4"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
               </div>`;
        card.innerHTML = `
            ${imgHtml}
...
/**
 * Remove or rewrite problematic URLs generated by the LLM:
 * - Strip any absolute localhost / 127.0.0.1 base (convert to relative path)
 * - Remove markdown links pointing to /developers/ (internal admin page)
 */
function sanitizeAiLinks(text) {
    if (!text) return '';
    // 1. Convert absolute dev-server URLs → relative paths
    //    e.g. http://127.0.0.1:8000/properties/slug/ → /properties/slug/
    text = text.replace(/https?:\/\/(127\.0\.0\.1|localhost)(:\d+)?(\S*)/g, '$3');
    // 2. Remove markdown links whose URL resolves to /developers/ (not user-facing)
    //    e.g. [Developers](/developers/) or [Developers](http://…/developers/)
    text = text.replace(/\[[^\]]*\]\(\/developers\/[^)]*\)/g, '');
    // 3. Strip bare /developers/ URLs that appear without markdown syntax
    text = text.replace(/\bhttps?:\/\/[^\s)"']*\/developers\/[^\s)"']*/g, '/properties/');
    return text;
}
